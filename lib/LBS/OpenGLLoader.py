from lib.LBS.Model import RawModel, Model
...
class OpenGLLoader:
    ...
    def load_to_VAO(self, model: Model) -> RawModel:
        vaoID = self.__create_VAO()
        indices = model.faces.flatten('C')
        vertices = model.vertices.flatten('C')
        texture_coords = model.vertex_texture_coords.flatten('C')
        normals = model.vertex_normals.flatten('C')
        vertex_joints = model.vertex_joints.flatten('C')
        weights = model.vertex_joints_weights.flatten('C')
        self.__bind_index_buffer(indices)
        self.__store_data_in_attribute_list(attribute_number=0, coordinate_size=3, data=vertices)
        self.__store_data_in_attribute_list(attribute_number=1, coordinate_size=2, data=texture_coords)
        self.__store_data_in_attribute_list(attribute_number=2, coordinate_size=3, data=normals)
        self.__store_data_in_int_attribute_list(attribute_number=3, coordinate_size=3, data=vertex_joints)
        self.__store_data_in_attribute_list(attribute_number=4, coordinate_size=3, data=weights)
        self.__unbind_VAO()
