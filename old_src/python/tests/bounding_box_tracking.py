...
def bbox2indices(bbox, img_shape):
    x0, x1, y0, y1 = bbox
    x0 = max(0, int(x0))
    y0 = max(0, int(y0))
    x1 = min(img_shape[1] - 1, int(x1))
    y1 = min(img_shape[2] - 1, int(y1))
    return x0, x1, y0, y1
...
def draw_bb(img_t: torch.Tensor, bbox: np.array):
    ...
    x0, x1, y0, y1 = bbox2indices(bbox, img_t.shape)
...
def crop_tensor(x: torch.Tensor, bbox: np.array):
    x0, x1, y0, y1 = bbox2indices(bbox, x.shape)
    return x[:, x0:x1, y0:y1].clone()


def main():
    # TODO: to make this work, make sure that this points to a valid video file
    video_file = osp.join('data', 'video_samples', 'workout.mp4')
